  ...
  useEffect(() => {
    const fetchItem = async () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          const itemData = { id: snapshot.key!, ...snapshot.val() } as Item;
          ...
          // Fetch owner profile
          const userRef = ref(db, `users/${itemData.userId}`);
          const userSnap = await get(userRef);
          if (userSnap.exists()) {
            setOwnerProfile(userSnap.val());
          }
  ...
  const fetchSimilarItems = async (currentItem: Item) => {
    try {
      const q = rtdbQuery(
        ...
        equalTo(currentItem.category)
      );
      const snapshot = await get(q);
      const items: Item[] = [];
      snapshot.forEach((child) => {
        const data = child.val();
        if (data.status === 'approved' && child.key !== currentItem.id) {
          items.push({ id: child.key!, ...data });
        }
      });
      setSimilarItems(items.slice(0, 3));
    } catch (error) {
              ...
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>{format(item.createdAt ? new Date(item.createdAt) : new Date(), 'MMMM d, yyyy')}</span>
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="p-4 space-y-2">
                          ...
                          <div className="flex items-center gap-1 text-xs text-slate-400">
                            ...
                            <span>{format(similarItem.createdAt ? new Date(similarItem.createdAt) : new Date(), 'MMM d, yyyy')}</span>
                          </div>
                        </div>
                      </Link>
                    ))}
