...
import { ref, onValue, get, query as rtdbQuery, orderByChild, equalTo } from 'firebase/database';
...
export default function Home() {
  ...
  useEffect(() => {
    const itemsRef = rtdbQuery(ref(db, 'items'), orderByChild('status'), equalTo('approved'));

    const unsubscribe = onValue(itemsRef, (snapshot) => {
      const itemsData: Item[] = [];
      snapshot.forEach((child) => {
        itemsData.push({ id: child.key!, ...child.val() } as Item);
      });
      itemsData.sort((a, b) => {
    ...
    const fetchStats = async () => {
      try {
        const itemsSnap = await get(rtdbQuery(ref(db, 'items'), orderByChild('status'), equalTo('approved')));
        let itemsCount = 0;
        itemsSnap.forEach(() => { itemsCount++; });

        const matchesSnap = await get(rtdbQuery(ref(db, 'matches'), orderByChild('status'), equalTo('confirmed')));
        let matchesCount = 0;
        matchesSnap.forEach(() => { matchesCount++; });

        setStats({
          reported: itemsCount,
          matched: matchesCount
        });
      } catch (error) {
