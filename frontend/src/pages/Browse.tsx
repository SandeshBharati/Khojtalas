...
export default function Browse() {
  ...
  useEffect(() => {
    ...
    const q = rtdbQuery(itemsRef, orderByChild('status'), equalTo('approved'));
    
    const unsubscribe = onValue(q, (snapshot) => {
      const itemsData: Item[] = [];
      snapshot.forEach((child) => {
        itemsData.push({ id: child.key!, ...child.val() });
      });
      // Sort by createdAt descending (RTDB doesn't support multi-field ordering)
      itemsData.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
      setItems(itemsData);
      setLoading(false);
    });
