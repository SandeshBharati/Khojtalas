  ...
  const renderStepper = () => (
    <div className="flex items-center justify-between mb-8 px-2">
      {[
        ...
        { step: 3, label: 'Location & Time' },
        { step: 4, label: 'Contact' }
      ].map((s, index) => (
        <React.Fragment key={s.step}>
          <div className="flex items-center gap-3">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${currentStep >= s.step ? 'bg-[#FF7A3D] text-white' : 'bg-gray-100 text-gray-500'}`}>
              {s.step}
            </div>
            <span className={`text-sm hidden sm:block ${currentStep >= s.step ? 'text-gray-900 font-medium' : 'text-gray-500'}`}>
              {s.label}
            </span>
          </div>
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-left space-y-2">
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Report Lost Item</h2>
          <p className="text-slate-500 text-lg">Fill in the details to help find your lost item.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-6 lg:p-8 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
            ...
            <button
              ...
              className="bg-[#FF7A3D] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#FF7A3D]/90 transition-all shadow-md flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
